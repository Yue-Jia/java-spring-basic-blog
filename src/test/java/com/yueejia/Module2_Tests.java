...
//                        "Sarah Holderness", new Date())
    ...
    public void task_2() {
        ...
        try {
            ...
            blogPostList = (List<BlogPost>)method.invoke(spyRepository);
        } catch (Exception e) {
            //e.printStackTrace();
        }
        ...
        assertNotNull(message, blogPostList);

        message = "Task 2: The method `getAllPosts()` does not return the correct `List`.";
        assertEquals(message, spyRepository.findAll(), blogPostList);
    }
    ...
    public void task_5() {
        ...
        Mockito.when(spyRepository.findAll()).thenReturn(ALL_Blog_POSTS);
    ...
    public void task_6() {
        ...
        ModelMap modelMap = Mockito.mock(ModelMap.class);
        Mockito.when(spyRepository.findAll()).thenReturn(ALL_Blog_POSTS);
        Mockito.when(modelMap.put("posts", ALL_Blog_POSTS)).thenReturn(null);
        ...
        try {
            Mockito.verify(modelMap).put("posts", ALL_Blog_POSTS);
            calledPut = true;
        } catch (Error e) {
            //e.printStackTrace();
        }
    ...
    public void task_8() {
        ...
        message = "Task 8: "+ ALL_Blog_POSTS.size()+" `<h2>` tags should have been generated by the Thymeleaf each loop with an `<h2>` tag inside.";
        assertTrue(message,
                h2Elements.size() == ALL_Blog_POSTS.size());

        // Check each h2 content
        for (int i=0; i < h2Elements.size(); i++) {
            Element element = h2Elements.get(i);
            message = "Task 8: The `<h2>` tag displays the title \"" + element.html() + "\" instead of \"" + ALL_Blog_POSTS.get(i).getTitle() + "\".";
            assertEquals(message, ALL_Blog_POSTS.get(i).getTitle(), element.html());
        }
    }
}
